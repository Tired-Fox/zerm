...
def fetch_xterm_colors():
    with open("xterm.json", "+w", encoding="utf-8") as f:
        f.write("{")
        for i, (name, color) in enumerate(xterm_colors):
            result = requests.get(URL + f"hex={color}")
            if result.status_code == 200:
                data = result.json()
                f.write(f'  "{name}:{i}": {dumps(data)},')
                print(name, data["name"]["value"], color)
            else:
                f.write(f'  "{name}:{i}": null,')
                print(name, "???", color)
        f.write("}")

def build_zig_xterm_enum():
    with open("xterm.json", "+r", encoding="utf-8") as f:
        named_colors = load(f)
    ...
    for name, data in named_colors.items():
        ...
        if rgb not in colors:
            colors.add(rgb)
            used_colors[name] = index

    print("pub const XTerm = enum(u8) {")
    for color in used_colors:
        print(f'    {color} = {used_colors[color]},')
    print("""
    pub fn format(self: @This(), comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        const code = @enumToInt(self);
